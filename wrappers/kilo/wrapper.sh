#!/usr/bin/env bash
#
# Agent Guard — Kilo CLI Wrapper
#
# Mandatory agent isolation enforcement for the Kilo Code CLI.
# Every invocation of `kilo` inside an Agent Guard managed repository is
...
#   cp <path-to>/wrappers/kilo/wrapper.sh <bin_dir>/kilo
#   chmod +x <bin_dir>/kilo
#
# Emergency bypass (use only for debugging/recovery):
#   AG_WRAPPER_BYPASS=1 kilo ...
#   AG_WRAPPER_BYPASS=1 kilo ...  # legacy alias
#
set -euo pipefail
...
if [[ "${AG_WRAPPER_BYPASS:-}" == "1" || "${AG_WRAPPER_BYPASS:-}" == "1" ]]; then
    # REAL_KILO is resolved later; bypass is handled after config load.
    # We need it now, so perform a minimal resolution.
    _AG_REAL_KILO="${AG_KILO_REAL:-${AG_KILO_REAL:-}}"
    if [[ -z "${_AG_REAL_KILO}" ]]; then
        # Try common locations.
        _AG_REAL_KILO_CANDIDATES=("${HOME}/.kilocode/bin/kilo.real" "$(command -v kilo.real 2>/dev/null || true)")
        for candidate in "${_AG_REAL_KILO_CANDIDATES[@]}"; do
            if [[ -n "${candidate}" && -x "${candidate}" ]]; then
                _AG_REAL_KILO="${candidate}"
                break
            fi
        done
    fi
    if [[ -z "${_AG_REAL_KILO}" || ! -x "${_AG_REAL_KILO}" ]]; then
        echo "❌ AG WRAPPER: cannot locate real kilo binary for bypass." >&2
        exit 1
    fi
    exec "${_AG_REAL_KILO}" "$@"
fi
...
#
#   kilo --slot kilo3            # acquire (or adopt) slot kilo3, then launch
#   AGENT_GUARD_SLOT=kilo3 kilo  # same, via environment variable
#
# The flag is consumed by the wrapper and never forwarded to the real CLI.
# If the Kilo CLI ever introduces its own --slot flag, use AGENT_GUARD_SLOT.
...
if [[ $# -gt 0 ]]; then
    ...
    while [[ $# -gt 0 ]]; do
        case "$1" in
            --slot)
                if [[ -z "${2:-}" ]]; then
                    echo "❌ AG WRAPPER: --slot requires an identity (ex: kilo3)." >&2
                    exit 1
                fi
                _AG_SLOT="$2"
                shift 2
...
# `exec` into kilo.real, so the lease stays bound to the agent process
...
# `kilo` invocation from inside an existing agent session): these variables
...
_ag_load_config() {
    ...
    _AG_BIN_DIR="$(bash "${config_bin}" get wrappers.kilo.bin_dir "${HOME}/.kilocode/bin" 2>/dev/null || echo "${HOME}/.kilocode/bin")"
    _AG_REAL_BIN_NAME="$(bash "${config_bin}" get wrappers.kilo.real_bin 'kilo.real' 2>/dev/null || echo 'kilo.real')"
    ...
    # Resolve real kilo binary.
    if ! _AG_REAL_KILO="$(try_source_real_kilo "${_AG_BIN_DIR}" "${_AG_REAL_BIN_NAME}")"; then
        # Fallback: search in PATH.
        local path_candidate
        path_candidate="$(command -v "${_AG_REAL_BIN_NAME}" 2>/dev/null || true)"
        if [[ -n "${path_candidate}" && -x "${path_candidate}" ]]; then
            _AG_REAL_KILO="${path_candidate}"
        fi
    fi
...
if ! _ag_load_config; then
    if _ag_looks_like_main_repo; then
        ...
        echo "     AG_WRAPPER_BYPASS=1 kilo ..." >&2
        exit 1
    fi
    ...
    _AG_REAL_KILO="${AG_KILO_REAL:-${AG_KILO_REAL:-}}"
    if [[ -z "${_AG_REAL_KILO}" ]]; then
        for candidate in "${HOME}/.kilocode/bin/kilo.real" "$(command -v kilo.real 2>/dev/null || true)" "$(command -v kilo 2>/dev/null || true)"; do
            if [[ -n "${candidate}" && -x "${candidate}" ]]; then
                _AG_REAL_KILO="${candidate}"
                break
            fi
    ...
    if [[ -n "${_AG_REAL_KILO}" && -x "${_AG_REAL_KILO}" ]]; then
        exec "${_AG_REAL_KILO}" "$@"
    fi
    echo "❌ AG WRAPPER: cannot locate real kilo binary." >&2
    exit 1
fi
...
if [[ -z "${_AG_REAL_KILO}" || ! -x "${_AG_REAL_KILO}" ]]; then
    echo "❌ AG WRAPPER: real kilo binary not found at ${_AG_BIN_DIR}/${_AG_REAL_BIN_NAME}" >&2
    exit 1
fi
...
_ag_scan_proc_once() {
    ...
    parsed="$(ps -eo pid,ppid,comm,args 2>/dev/null | tail -n +2 | awk '
    {
        ...
        ppid_map[pid] = ppid;
        if (comm == "kilocode" || comm == "claude" || comm == "gemini" || comm == "grok" || comm == "cursor" || comm == "antigravity" || comm == "kiro" || comm == "kilo" || args ~ /(^|[^[:alnum:]_])(kilocode|claude|gemini|grok|cursor|antigravity|kiro|kilo)([^[:alnum:]_]|$)/) {
            agents[pid] = 1;
        }
    }
    END {
...
# shell/wrapper itself as an agent (e.g. its argv contains a .kilocode path);
...
_ag_slots_snapshot() {
    if [[ -z "${_AG_SLOTS_SNAPSHOT}" ]]; then
        ...
        # package (e.g. ~/.kilocode/bin/kilo), where SCRIPT_DIR/../.. is not
        # the package root.
        local _slots_bin
...
_ag_find_free_kilo_worktree() {
    for prefix in ${_AG_KNOWN_IDENTITIES}; do
        [[ "${prefix}" == "kilo" ]] || continue
        local wt_prefix
        wt_prefix="$(bash "${_AG_CONFIG_BIN}" get "identities.${prefix}.worktree_prefix" '' 2>/dev/null || true)"
...
if ! _ag_have_lease; then
    ...
    if [[ -n "${_AG_SLOT}" ]]; then
        if [[ ! "${_AG_SLOT}" =~ ^[a-z]+[0-9]+$ ]]; then
            echo "❌ AG WRAPPER: invalid slot '${_AG_SLOT}' (expected e.g. kilo3)." >&2
            exit 1
        fi
        ...
        # Each wrapper family manages its own identity prefix (the Kilo wrapper
        # takes kiloN slots, never claudeN/geminiN). The expected prefix is
        # configurable for upstream projects with different naming.
        _ag_wrapper_prefix="$(bash "${_AG_CONFIG_BIN}" get "wrappers.kilo.identity_prefix" "kilo" 2>/dev/null || echo "kilo")"
        ...
        _ag_default_role="$(bash "${_AG_CONFIG_BIN}" get "wrappers.kilo.default_role" "ia-a" 2>/dev/null || echo "ia-a")"
        ORIGINAL_ARGS=("$@")
        set --
    ...
    if [[ "${CWD}" == "${_AG_MAIN_REPO}" ]]; then
        ...
        _AG_RESUMABLE_WORKTREE="$(_ag_find_resumable_worktree "kilo" 2>/dev/null || true)"
        if [[ -n "${_AG_RESUMABLE_WORKTREE}" ]]; then
            ...
            _AG_FREE_WORKTREE="$(_ag_find_free_kilo_worktree 2>/dev/null || true)"
            if [[ -n "${_AG_FREE_WORKTREE}" ]]; then
                ...
                default_role="$(bash "${_AG_CONFIG_BIN}" get "wrappers.kilo.default_role" "ia-a" 2>/dev/null || echo "ia-a")"
                echo "🔄 AG WRAPPER: no free worktree available; allocating new slot..." >&2
                ORIGINAL_ARGS=("$@")
                set --
                if ! source "${_AG_INIT_SCRIPT}" kilo "${default_role}" >"${_AG_WRAPPER_LOG}" 2>&1; then
        ...
        if _ag_worktree_has_live_agent "${CWD}"; then
            ...
            echo "   Start kilo from ${_AG_MAIN_REPO} to get a free worktree," >&2
    ...
    # Legacy aliases used by older Kilo CLI builds and by the Kilo Status
...
if [[ -f "${_ag_session_trace_script}" && -n "${_AG_WORKTREE:-}" && -n "${_AG_IDENTITY:-}" && -n "${_AG_BRANCH:-}" ]]; then
    ...
    # Start a background watcher to snapshot Kilo session metadata (title,
    ...
    # explicit checkpoint. The watcher is best-effort and never blocks Kilo.
    watch_interval="${AGENT_GUARD_KILO_WATCH_INTERVAL_SECONDS:-60}"
    if [[ "${watch_interval}" -gt 0 && -n "${_AG_WORKTREE:-}" ]]; then
        ...
        # Kilo process exits. This is a best-effort trace; losing its output is
        # acceptable.
        (
            ...
            source "${_ag_session_trace_script}" >/dev/null 2>&1
            _trace_watch_kilo_session "$$" "${_AG_WORKTREE}" "${_ag_session_trace_dir}" "${watch_interval}" "${AGENT_GUARD_KILO_WATCH_CHECKPOINT_INTERVAL_SECONDS:-300}" >/dev/null 2>&1
...
if _ag_is_foreign_worktree; then
    ...
    echo "   Change to your worktree or start kilo from ${_AG_MAIN_REPO}." >&2
    exit 1
fi
