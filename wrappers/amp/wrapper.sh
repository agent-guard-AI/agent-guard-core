#!/usr/bin/env bash
#
# Agent Guard — Amp CLI Wrapper
#
# Mandatory agent isolation enforcement for the Amp CLI.
# Every invocation of `amp` inside an Agent Guard managed repository is
...
#   cp <path-to>/wrappers/amp/wrapper.sh ~/.local/hmvip/bin/amp
...
if [[ "${AG_WRAPPER_BYPASS:-}" == "1" ]]; then
    # The full configured path is resolved later; bypass uses the canonical
    # installer location so it cannot recurse through ~/.local/bin/amp.
    # We need it now, so perform a minimal resolution.
    _AG_REAL_AMP="${AG_AMP_REAL:-}"
    if [[ -z "${_AG_REAL_AMP}" ]]; then
        # Try common locations.
        _AG_REAL_AMP_CANDIDATES=("${HOME}/.amp/bin/amp")
        for candidate in "${_AG_REAL_AMP_CANDIDATES[@]}"; do
            if [[ -n "${candidate}" && -x "${candidate}" ]]; then
                _AG_REAL_AMP="${candidate}"
                break
            fi
        done
    fi
    _AG_BYPASS_WRAPPER="$(readlink -f "${BASH_SOURCE[0]}" 2>/dev/null || printf '%s' "${BASH_SOURCE[0]}")"
    _AG_BYPASS_REAL="$(readlink -f "${_AG_REAL_AMP}" 2>/dev/null || printf '%s' "${_AG_REAL_AMP}")"
    if [[ -z "${_AG_REAL_AMP}" || ! -x "${_AG_REAL_AMP}" || "${_AG_BYPASS_REAL}" == "${_AG_BYPASS_WRAPPER}" ]]; then
        echo "❌ AG WRAPPER: cannot locate real amp binary for bypass." >&2
        exit 1
    fi
    exec "${_AG_REAL_AMP}" "$@"
fi
...
#
#   amp --slot amp3            # acquire (or adopt) slot amp3, then launch
#   AGENT_GUARD_SLOT=amp3 amp  # same, via environment variable
#
# The flag is consumed by the wrapper and never forwarded to the real CLI.
# If the Amp CLI ever introduces its own --slot flag, use AGENT_GUARD_SLOT.
...
if [[ $# -gt 0 ]]; then
    ...
    while [[ $# -gt 0 ]]; do
        case "$1" in
            --slot)
                if [[ -z "${2:-}" ]]; then
                    echo "❌ AG WRAPPER: --slot requires an identity (ex: amp3)." >&2
                    exit 1
                fi
                _AG_SLOT="$2"
                shift 2
...
# `exec` into amp, so the lease stays bound to the agent process
...
# `amp` invocation from inside an existing agent session): these variables
...
_ag_load_config() {
    ...
    _AG_BIN_DIR="$(bash "${config_bin}" get wrappers.amp.bin_dir "${HOME}/.local/hmvip/bin" 2>/dev/null || echo "${HOME}/.local/hmvip/bin")"
    _AG_REAL_BIN_NAME="$(bash "${config_bin}" get wrappers.amp.real_bin_path "${HOME}/.amp/bin/amp" 2>/dev/null || echo "${HOME}/.amp/bin/amp")"
    ...
    # Resolution order is explicit override, configured path, canonical Amp
    ...
    path_candidate="$(command -v amp 2>/dev/null || true)"
    for candidate in "${AG_AMP_REAL:-}" "${configured_real}" "${HOME}/.amp/bin/amp" "${path_candidate}"; do
        if _AG_REAL_AMP="$(_ag_accept_real_amp "${candidate}" 2>/dev/null)"; then
            break
        fi
    done
...
if ! _ag_load_config; then
    if _ag_looks_like_main_repo; then
        ...
        echo "     AG_WRAPPER_BYPASS=1 amp ..." >&2
        exit 1
    fi
    ...
    _AG_REAL_AMP=""
    if [[ -z "${_AG_REAL_AMP}" ]]; then
        for candidate in "${AG_AMP_REAL:-}" "${HOME}/.amp/bin/amp" "$(command -v amp 2>/dev/null || true)"; do
            if _AG_REAL_AMP="$(_ag_accept_real_amp "${candidate}" 2>/dev/null)"; then
                break
            fi
        done
    fi
    if [[ -n "${_AG_REAL_AMP}" && -x "${_AG_REAL_AMP}" ]]; then
        exec "${_AG_REAL_AMP}" "$@"
    fi
    echo "❌ AG WRAPPER: cannot locate real amp binary." >&2
    exit 1
fi
...
if [[ -z "${_AG_REAL_AMP}" || ! -x "${_AG_REAL_AMP}" ]]; then
    echo "❌ AG WRAPPER: real amp binary not found (configured path: ${_AG_REAL_BIN_NAME})." >&2
    exit 1
fi
...
_ag_is_management_command() {
    for arg in "$@"; do
        case "${arg}" in
            --version|-V|--help|-h|update|upgrade|login)
                return 0
                ;;
        esac
    done
...
_ag_scan_proc_once() {
    ...
    parsed="$(ps -eo pid,ppid,comm,args 2>/dev/null | tail -n +2 | awk '
    {
        ...
        ppid_map[pid] = ppid;
        if (comm == "kimi-code" || comm == "claude" || comm == "gemini" || comm == "grok" || comm == "cursor" || comm == "antigravity" || comm == "kiro" || comm == "amp" || args ~ /(^|[^[:alnum:]_])(kimi-code|claude|gemini|grok|cursor|antigravity|kiro|kimi|amp)([^[:alnum:]_]|$)/) {
            agents[pid] = 1;
        }
    }
    END {
...
_ag_slots_snapshot() {
    if [[ -z "${_AG_SLOTS_SNAPSHOT}" ]]; then
        ...
        # package, where SCRIPT_DIR/../.. is not the package root.
        local _slots_bin
...
_ag_find_free_amp_worktree() {
    for prefix in ${_AG_KNOWN_IDENTITIES}; do
        [[ "${prefix}" == "amp" ]] || continue
        local wt_prefix
        wt_prefix="$(bash "${_AG_CONFIG_BIN}" get "identities.${prefix}.worktree_prefix" 'hmvip-ia-amp' 2>/dev/null || echo 'hmvip-ia-amp')"
...
if ! _ag_have_lease; then
    ...
    if [[ -n "${_AG_SLOT}" ]]; then
        if [[ ! "${_AG_SLOT}" =~ ^[a-z]+[0-9]+$ ]]; then
            echo "❌ AG WRAPPER: invalid slot '${_AG_SLOT}' (expected e.g. amp3)." >&2
            exit 1
        fi
        ...
        # Each wrapper family manages its own identity prefix (the Amp wrapper
        # takes ampN slots, never claudeN/geminiN). The expected prefix is
        # configurable for upstream projects with different naming.
        _ag_wrapper_prefix="$(bash "${_AG_CONFIG_BIN}" get "wrappers.amp.identity_prefix" "amp" 2>/dev/null || echo "amp")"
        ...
        _ag_wt_prefix="$(bash "${_AG_CONFIG_BIN}" get "identities.${_ag_slot_prefix}.worktree_prefix" 'hmvip-ia-amp' 2>/dev/null || echo 'hmvip-ia-amp')"
    ...
    if [[ "${CWD}" == "${_AG_MAIN_REPO}" ]]; then
        ...
        _AG_RESUMABLE_WORKTREE="$(_ag_find_resumable_worktree "amp" 2>/dev/null || true)"
        if [[ -n "${_AG_RESUMABLE_WORKTREE}" ]]; then
            ...
            _AG_FREE_WORKTREE="$(_ag_find_free_amp_worktree 2>/dev/null || true)"
            if [[ -n "${_AG_FREE_WORKTREE}" ]]; then
                ...
                default_role="$(bash "${_AG_CONFIG_BIN}" get "wrappers.amp.default_role" "ia-a" 2>/dev/null || echo "ia-a")"
                echo "🔄 AG WRAPPER: no free worktree available; allocating new slot..." >&2
                ORIGINAL_ARGS=("$@")
                set --
                if ! source "${_AG_INIT_SCRIPT}" amp "${default_role}" >"${_AG_WRAPPER_LOG}" 2>&1; then
        ...
        if _ag_worktree_has_live_agent "${CWD}"; then
            ...
            echo "   Start amp from ${_AG_MAIN_REPO} to get a free worktree," >&2
    ...
    # Legacy aliases used by older Amp CLI builds and by the Amp Status
...
if [[ -f "${_ag_session_trace_script}" && -n "${_AG_WORKTREE:-}" && -n "${_AG_IDENTITY:-}" && -n "${_AG_BRANCH:-}" ]]; then
    ...
    # Start a background watcher while the Amp frontend process is alive. Amp
    # does not expose Kimi's state.json format, so this records periodic trace
    # checkpoints without importing CLI-specific state. The comm/args check
    # prevents a recycled PID from keeping the watcher alive.
    watch_interval="${AGENT_GUARD_AMP_WATCH_INTERVAL_SECONDS:-60}"
    if [[ "${watch_interval}" -gt 0 && -n "${_AG_WORKTREE:-}" ]]; then
        ...
        # Amp process exits. This is a best-effort trace; losing its output is
        # acceptable.
        (
            ...
            source "${_ag_session_trace_script}" >/dev/null 2>&1
            amp_pid="$$"
            checkpoint_interval="${AGENT_GUARD_AMP_WATCH_CHECKPOINT_INTERVAL_SECONDS:-300}"
            last_checkpoint="$(date +%s)"
            while kill -0 "${amp_pid}" >/dev/null 2>&1; do
                sleep "${watch_interval}" || true
                amp_comm="$(ps -p "${amp_pid}" -o comm= 2>/dev/null || true)"
                amp_args="$(ps -p "${amp_pid}" -o args= 2>/dev/null || true)"
                if [[ "${amp_comm}" != "amp" && ! "${amp_args}" =~ (^|[[:space:]/])amp([[:space:]]|$) ]]; then
                    break
                fi
                now="$(date +%s)"
                _trace_heartbeat "amp watcher heartbeat" >/dev/null 2>&1 || true
                if [[ $((now - last_checkpoint)) -ge ${checkpoint_interval} ]]; then
                    _trace_checkpoint "auto checkpoint from amp watcher" >/dev/null 2>&1 || true
                    last_checkpoint="${now}"
                fi
            done
            ...
            if [[ -n "${ag_init_stub}" && -f "${ag_init_stub}" && -n "${_AG_WORKTREE:-}" ]]; then
                ...
                if source "${ag_init_stub}" >/dev/null 2>&1; then
                    ...
                    if [[ -n "${amp_identity}" ]]; then
                        ...
                        # Session status via the public facade (F5B) instead of
                        # the internal storage helper. Fail-open: empty status
                        # simply skips the release attempt (previous behavior
                        # when the field could not be read).
                        amp_session_status="$(_ag_slot_field "${amp_identity}" status)"
...
if _ag_is_foreign_worktree; then
    ...
    echo "   Change to your worktree or start amp from ${_AG_MAIN_REPO}." >&2
    exit 1
fi
